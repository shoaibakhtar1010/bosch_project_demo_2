...
Using `Annotated` avoids calling FastAPI dependency functions (like `File()`)
at import time, which Ruff's B008 rule flags.  See the ``predict`` route
definition below for an example.
"""
...
# Third‑party imports (alphabetical, case‑insensitive)
...
@app.post("/predict")
async def predict(
    iris: Annotated[UploadFile, File(...)],
